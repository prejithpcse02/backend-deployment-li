...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    reviewed_nickname = serializers.CharField(source="reviewed_user.nickname", read_only=True)

    class Meta:
        model = Review
        fields = [
            'id', 'reviewer', 'reviewed_user', 'reviewed_product', 'rating', 'review_text', 
            'product_image', 'parent_review', 'created_at', 
            'reviewer_username', 'reviewer_nickname', 
            'reviewed_username', 'reviewed_nickname'
        ]
