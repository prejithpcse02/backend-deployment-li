...
@receiver(post_save, sender=Listing)
def create_listing_sold_notification(sender, instance, **kwargs):
    """
    Create notification when a listing is marked as sold
    """
    if instance.tracker.has_changed('status') and instance.status == 'sold':
        NotificationService.create_listing_sold_notification(instance, instance.buyer)
